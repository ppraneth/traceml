...
class TraceMLInitConfig:
    ...
    patch_h2d:
        Whether host-to-device transfer timing patching is enabled.
    source:
...
def _apply_requested_patches(config: TraceMLInitConfig) -> None:
    ...
    try:
        ...
        if config.patch_h2d:
            from traceml.utils.patches.h2d_auto_timer_patch import patch_h2d
...
def init(
    *,
    ...
    patch_h2d:
        Selective-mode-only override controlling H2D transfer timing patching.
